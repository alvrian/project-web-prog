    ...
    <div class="accordion-item">
      ...
      <div class="accordion-body d-grid gap-3">
        ...
        <div class="col-5 row" style="border-left: 1px solid grey">
          ...
          @if($d->Status == 'Active')
          <form method="POST" action="{{ route('farmer.subsManagePause') }}" class="pauseForm col-2"
          ...
          <form method="POST" action="{{ route('farmer.subsManageCancel') }}" class="cancelForm col-2"
          data-subscription-id="{{ $d->SubscriptionID }}">
          @csrf
          <input type="hidden" name="subscriptionID" value="{{ $d->SubscriptionID }}">
          <button class="btn mt-2" type="submit" style="background-color: #E74C3C; color:white;">
          Cancel
          </button>
          </form>
          @elseif($d->Status == 'Postponed')
          <form method="POST" action="{{ route('farmer.subsManageResume') }}" class="resumeForm"
          data-subscription-id="{{ $d->SubscriptionID }}">
          @csrf
          <input type="hidden" name="subscriptionID" value="{{ $d->SubscriptionID }}">
